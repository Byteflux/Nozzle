...
function import {
    ...
    file="${1}.java"
    target="$workdir/Paper/Paper-Server/src/main/java/$nms/$file"
...
function importLibrary {
    ...
    for file in "$@"; do
        file="$prefix/$file"
        target="$workdir/Paper/Paper-Server/src/main/java/${file}"
...
files=$(cat "$basedir/Paper-Server-Patches/"* | grep "+++ b/src/main/java/net/minecraft/server/" | sort | uniq | sed 's/\+\+\+ b\/src\/main\/java\/net\/minecraft\/server\///g' | sed 's/.java//g')

nonnms=$(grep -R "new file mode" -B 1 "$basedir/Paper-Server-Patches/" | grep -v "new file mode" | grep -oE "net\/minecraft\/server\/.*.java" | grep -oE "[A-Za-z]+?.java$" --color=none | sed 's/.java//g')
...
for f in $files; do
	...
	if [ "$?" == "1" ]; then
		if [ ! -f "$workdir/Paper/Paper-Server/src/main/java/net/minecraft/server/$f.java" ]; then
...
set -e
cd "$workdir/Paper/Paper-Server/"
if [[ $($gitcmd status --porcelain) ]]; then
