...
savePatches "$workdir/Paper/Paper-API" "Nozzle-API"
if [ -f "$basedir/Nozzle-API/.git/patch-apply-failed" ]; then
    echo "$(color 1 31)[[[ WARNING ]]] $(color 1 33)- Not saving Nozzle-Server as it appears Nozzle-API did not apply clean.$(colorend)"
    ...
    echo "$(color 1 33)Otherwise, rerun ./nozzle patch to have a clean Nozzle-API apply so the latest Nozzle-Server can build.$(colorend)"
else
    savePatches "$workdir/Paper/Paper-Server" "Nozzle-Server"
fi
) || exit 1
